...
import backbones.efficientnet.effnetv2_configs as effnetv2_configs
import backbones.efficientnet.effnetv2_hparams as hparams
import backbones.efficientnet.effnetv2_utils as utils
...
class MBConvBlock(keras.layers.Layer):
    ...
    def call(self, inputs, training, survival_prob=None):
        ...
        x = utils.fixed_padding(x, self._block_args.kernel_size, shifts=shifts)
        ...
        if self._mconfig.conv_dropout and self._block_args.expand_ratio > 1:
            x = keras.layers.Dropout(self._mconfig.conv_dropout)(
                x, training=training)
    ...
    def call(self, inputs, training, survival_prob=None):
        ...
        if self._block_args.expand_ratio != 1:
            ...
            x = utils.fixed_padding(
                x, self._block_args.kernel_size, shifts=shifts)
            x = self._act(self._norm0(self._expand_conv(x), training=training))
        logging.debug('Expand shape: %s', x.shape)
        ...
        if self._mconfig.conv_dropout and self._block_args.expand_ratio > 1:
            x = keras.layers.Dropout(self._mconfig.conv_dropout)(x, training)
...
class Head(keras.layers.Layer):
    ...
    def __init__(self, mconfig, name=None):
        ...
        self._avg_pooling = keras.layers.GlobalAveragePooling2D(
            data_format=mconfig.data_format)

        if mconfig.dropout_rate > 0:
            self._dropout = keras.layers.Dropout(mconfig.dropout_rate)
        else:
            self._dropout = None
...
class EffNetV2Model(keras.Model):
    """A class implements keras.Model.
    ...
    def summary(self, input_shape=(224, 224, 3), **kargs):
        ...
        model = keras.Model(inputs=[x], outputs=self.call(x, training=True))
